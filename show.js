...
const SHOW_ID=Number(params.get("id"));
let episodes=[],show=null,config=null,watched=TVT.getProgress(SHOW_ID);
const seasonsEl=document.querySelector("#seasons");

function trackedEpisodes(){
  return TVT.filterTrackedEpisodes(episodes,config);
}

function updateStats(){
  const regular=trackedEpisodes();
  ...
  const remaining=aired.filter(e=>!watched[String(e.id)])
  ...
  document.querySelector("#nextBox").innerHTML=next
    ...
    : nextUpcoming
      ...
      : `<strong>Complete!</strong><span>You've watched every tracked aired episode.</span>`;

  document.querySelectorAll(".season").forEach(section=>{
    const sn=Number(section.dataset.season);
    const eps=aired.filter(e=>e.season===sn);
    const checked=eps.filter(e=>watched[String(e.id)]).length;
    const future=upcoming.filter(e=>e.season===sn).length;
    section.querySelector(".season-count").textContent=
      `${checked} / ${eps.length} watched${future?` · ${future} upcoming`:""}`;
  });
}
...
function applyFilters(){
  ...
  document.querySelectorAll(".episode").forEach(row=>{
    const watchedRow=row.dataset.watched==="true";
    const upcoming=row.dataset.upcoming==="true";
    const text=row.dataset.search.includes(term);

    const match=filter==="all" ||
      (filter==="watched"&&watchedRow) ||
      (filter==="unwatched"&&!watchedRow&&!upcoming) ||
...
function render(){
  ...
  trackedEpisodes().forEach(ep=>(grouped[ep.season]||=[]).push(ep));

  Object.entries(grouped).forEach(([sn,eps])=>{
    const section=document.createElement("section");
    section.className="season";
    section.dataset.season=sn;
    ...
    body.className="episodes";

    eps.forEach(ep=>{
      const aired=TVT.isAired(ep);
      const checked=!!watched[String(ep.id)];
      const row=document.createElement("label");

      row.className="episode"+(checked?" watched":"")+(!aired?" upcoming":"");
      row.dataset.watched=checked?"true":"false";
      row.dataset.upcoming=aired?"false":"true";
      ...
      box.addEventListener("change",()=>{
        if(box.checked) watched[String(ep.id)]=true;
        else delete watched[String(ep.id)];

        row.dataset.watched=box.checked?"true":"false";
        row.classList.toggle("watched",box.checked);
        TVT.saveProgress(SHOW_ID,watched);
        updateStats();
        applyFilters();
      });
...
async function init(){
  try{
    ...
    const tracked=trackedEpisodes();
    const seasons=new Set(tracked.map(e=>e.season)).size;
    const tracking=config?.startSeason?` · tracking from Season ${config.startSeason}`:"";

    document.querySelector("#showMeta").textContent=
      `${years}${years?" · ":""}${seasons} tracked season${seasons===1?"":"s"}${tracking}`;
